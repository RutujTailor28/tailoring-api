...
exports.register = asyncHandler(async (req, res, next) => {
    const {name, phone, email = '', address, measurement, customerId} = req.body;
    console.log("measurement ==> ",measurement);
    console.log("measurement type ==> ",typeof measurement);
    const userId = req.user.id;

    // console.log('userId => ',userId,' | name => ',name,' | phone => ',phone,' | address => ',address,' | measurement => ',measurement, ' | profileImage => ',req.files);
...
exports.searchCustomer = asyncHandler(async (req, res, next) => {
    ...
    const customerData = await Customer.find({
        ...
        }, {customerPhone: {$regex: searchString, $options: "i"}}], $and: [{userId: req.user.id}]
    });
...
const getCustomerDataStructure = (customerData) => {
    const newResponse = customerData && customerData.length && customerData.map((data) => {
        return {
